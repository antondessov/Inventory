...
void UInv_InventoryGrid::NativeOnInitialized()
{
	...
	InventoryComponent->OnInventoryToggled.AddDynamic(this, &ThisClass::OnInventoryMenuToggled);
...
{
	UnHighlightSlots(LastHighlightedIndex, LastHighlightedDimensions);
	UInventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns, [GridSlotState = GridSlotState](UInv_GridSlot* GridSlot)
	{
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		// Should we consume the hover item's stack? (Room in the clicked slot >= HoveredStackCount)
		...
		// Should we fill in the stack of the clicked item? (and not consume the hover item)
...
void UInv_InventoryGrid::CreateItemPopUp(const int32 GridIndex)
{
	...
	const FVector2D MousePotion = UWidgetLayoutLibrary::GetMousePositionOnViewport(GetOwningPlayer());
	CanvasSlot->SetPosition(MousePotion - ItemPopUpOffset);
